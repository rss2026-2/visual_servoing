...
def run_trial_bayesian(num_iterations, trial_name):
    ...
    for i in range(1, max_filters+1):
        param_bounds[f"switch_{i}"] = (1,2.999)
        param_bounds[f"size_{i}"] = size_bounds
        param_bounds[f"iter_{i}"] = iter_bounds
    ...
    for i in range(num_iterations):
        ...
        optimizer.register(params = new_params, target = new_avg_min_mean)

        if new_avg_min_mean > trial_data["avg_min_mean"]: #if there is a net increase
...
def run_trials(num_trials, num_iterations, trial_name, trial_type = "random", plot = True):
    ...
    for i in range(1, num_trials+1):
        ...
        if trial_type == "random":
            print(run_trial_random(num_iterations, trial_name+"_"+str(i)))
        elif trial_type == "bayesian":
            print(run_trial_bayesian(num_iterations, trial_name+"_"+str(i)))
...
if __name__ == '__main__':
    ...
    run_trials(3, 100, "worm", "bayesian", True)
    pass